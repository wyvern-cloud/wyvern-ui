...
class DIDCommWorker {
  ...
  async getDidSecrets() {
    try {
      const secrets = await (this.didcomm.secretsResolver.get_secrets());
      console.log(secrets);
      this.postMessage({
        type: "didSecrets",
        payload: {
          did: this.didForMediator,
          mediatedDid: this.did,
          secrets: secrets,
        },
      });
    } catch (error) {
      console.error("Error getting DID secrets:", error);
      this.postMessage({ type: "error", payload: { error: "Failed to get DID secrets" } });
    }
  }
